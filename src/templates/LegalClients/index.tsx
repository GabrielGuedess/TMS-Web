...
import { DataTableLegalClients } from 'components/organisms/DataTableLegalClients';

import { type LegalClientsProps } from './types';

const LegalClientsRef: ForwardRefRenderFunction<
  HTMLElement,
  LegalClientsProps
> = ({ ...props }, ref) => (
  <section
    ...
    {...props}
  >
    <Breadcrumb pageName="Clientes Jurídicos" />
    ...
    <GlowCapture className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-[1.875rem]">
      ...
      <CardDataStats
        ...
        icon={UsersIcon}
        isPositive={false}
        title="Total LegalClients"
      />
    </GlowCapture>

    <Container>
      <DataTableLegalClients />
    </Container>
  </section>
);

export const LegalClients = forwardRef(LegalClientsRef);
