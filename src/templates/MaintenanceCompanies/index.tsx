...
import { DataTableMaintenanceCompanies } from 'components/organisms/DataTableMaintenanceCompanies';

import { type MaintenanceCompaniesProps } from './types';

const MaintenanceCompaniesRef: ForwardRefRenderFunction<
  HTMLElement,
  MaintenanceCompaniesProps
> = ({ ...props }, ref) => (
  <section
    ref={ref}
    ...
    {...props}
  >
    <Breadcrumb pageName="Empresas de Manutenção" />
    ...
    <GlowCapture className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-[1.875rem]">
      ...
      <CardDataStats
        ...
        icon={UsersIcon}
        isPositive={false}
        title="Total MaintenanceCompanies"
      />
    </GlowCapture>

    <Container>
      <DataTableMaintenanceCompanies />
    </Container>
  </section>
);

export const MaintenanceCompanies = forwardRef(MaintenanceCompaniesRef);
