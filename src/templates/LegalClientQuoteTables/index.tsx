...
import { DataTableLegalClientQuoteTables } from 'components/organisms/DataTableLegalClientQuoteTables';

import { type LegalClientQuoteTablesProps } from './types';

const LegalClientQuoteTablesRef: ForwardRefRenderFunction<
  HTMLElement,
  LegalClientQuoteTablesProps
> = ({ ...props }, ref) => (
  <section
    ref={ref}
    ...
    {...props}
  >
    <Breadcrumb pageName="Tabela de Cotação de Cliente Jurídico" />
    ...
    <GlowCapture className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-[1.875rem]">
      ...
      <CardDataStats
        ...
        icon={UsersIcon}
        isPositive={false}
        title="Total LegalClientQuoteTables"
      />
    </GlowCapture>

    <Container>
      <DataTableLegalClientQuoteTables />
    </Container>
  </section>
);

export const LegalClientQuoteTables = forwardRef(LegalClientQuoteTablesRef);
