...
import {
  useGetAllRecipientQuery,
  type RecipientWhereInput,
  type RecipientUpdateManyInput,
  useDeleteManyRecipientsMutation,
  useUpdateManyRecipientsMutation,
...
export const DataTableRecipients = ({ ...props }: DataTableRecipientsProps) => {
  const [isFetchingRecipients, setIsFetchingRecipients] = useState(false);
  const [isFetchingUpdateRecipients, setIsFetchingUpdateRecipients] =
    useState(false);
  const [isFetchingDeleteRecipients, setIsFetchingDeleteRecipients] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<RecipientProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<RecipientProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyRecipientsMutation();
  const [deleteRecipients] = useDeleteManyRecipientsMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<RecipientProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as RecipientKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: RecipientProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<RecipientUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<RecipientProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    const { data: result } = await refetch({
      ...
      where: { legal_person_id: { contains: search } },
    });

    setRowData(result.getAllRecipient ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllRecipient ?? []);

        return {
          ...fetchMoreResult,
          Recipients: [
            ...(previous.getAllRecipient ?? []),
            ...(fetchMoreResult.getAllRecipient ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateRecipients = async () => {
    setIsFetchingUpdateRecipients(true);

    try {
      const { data } = await update({
        variables: {
          updateManyRecipients: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyRecipients && data.updateManyRecipients.length > 1;

      toast.success(
        `Destinatário${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Destinatário!', {
      ...
      setIsFetchingUpdateRecipients(false);
    }
  };

  const handleDeleteRecipients = async () => {
    setIsFetchingDeleteRecipients(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteRecipients({
        variables: { deleteManyRecipients: ids },
      });

      const isPlural =
        data?.deleteManyRecipients && data?.deleteManyRecipients.length > 1;

      toast.success(
        `Destinatário${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Destinatário!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteRecipients(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<RecipientProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<RecipientProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as RecipientWhereInput;
    const keys = Object.keys(filter) as RecipientKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<RecipientProps, '__typename' | 'legalPersonId'>
        ] = {
    ...
    setRowData(data.getAllRecipient ?? []);
  };

  const rowClassRules: RowClassRules<RecipientProps> = {
  ...
  useEffect(() => {
    if (dataRecipients?.getAllRecipient ?? []) {
      setRowData(dataRecipients?.getAllRecipient ?? []);
    }
  }, [isLoading]);
        ...
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      isLoading={isFetchingUpdateRecipients}
                      isDisabled={isFetchingUpdateRecipients}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/recipients/new">
              <Button variant="label">Adicionar Destinatário</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingRecipients,
                    'opacity-0 pointer-events-none': !isFetchingRecipients,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/recipients/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteRecipients}
                              isDisabled={isFetchingDeleteRecipients}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataRecipients.totalRecipients / pageSize,
                )} página${
                  Math.ceil(dataRecipients.totalRecipients / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataRecipients?.totalRecipients} registro${
                  dataRecipients?.totalRecipients > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                length={Math.ceil(dataRecipients.totalRecipients / pageSize)}
              />
            </div>
          </>
        )}
