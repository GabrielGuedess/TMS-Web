...
import {
  ...
  useDeleteManyVehicleModelsMutation,
  useUpdateManyVehicleModelsMutation,
...
import {
  ...
  type VehicleModelKeyProps,
  type DataTableVehicleModelsProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableVehicleModels = ({
  ...props
}: DataTableVehicleModelsProps) => {
  const [isFetchingVehicleModels, setIsFetchingVehicleModels] = useState(false);
  const [isFetchingUpdateVehicleModels, setIsFetchingUpdateVehicleModels] =
    useState(false);
  const [isFetchingDeleteVehicleModels, setIsFetchingDeleteVehicleModels] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<VehicleModelProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<VehicleModelProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyVehicleModelsMutation();
  const [deleteVehicleModels] = useDeleteManyVehicleModelsMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<VehicleModelProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as VehicleModelKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: VehicleModelProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<VehicleModelUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<VehicleModelProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        editable: true,
        field: 'capacity_max',
        headerName: 'Capacidade Maxima',
        filterParams: filterParametersText,
      },
      {
        filter: true,
        editable: true,
        field: 'capacity_per_axle',
        filterParams: filterParametersText,
        headerName: 'Capacidade Maxima por Eixo',
      },
      {
        filter: true,
        editable: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingVehicleModels(true);
    setSelectedRows([]);
    ...
    setRowData(result.getAllVehicleModel ?? []);
    setIsFetchingVehicleModels(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllVehicleModel ?? []);

        return {
          ...fetchMoreResult,
          VehicleModels: [
            ...(previous.getAllVehicleModel ?? []),
            ...(fetchMoreResult.getAllVehicleModel ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateVehicleModels = async () => {
    setIsFetchingUpdateVehicleModels(true);

    try {
      const { data } = await update({
        variables: {
          updateManyVehicleModels: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyVehicleModels &&
        data.updateManyVehicleModels.length > 1;

      toast.success(
        `Modelo de Veículo${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Modelo de Veículo!', {
      ...
      setIsFetchingUpdateVehicleModels(false);
    }
  };

  const handleDeleteVehicleModels = async () => {
    setIsFetchingDeleteVehicleModels(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteVehicleModels({
        variables: { deleteManyVehicleModels: ids },
      });

      const isPlural =
        data?.deleteManyVehicleModels &&
        data?.deleteManyVehicleModels.length > 1;

      toast.success(
        `Modelo de Veículo${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Modelo de Veículo!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteVehicleModels(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<VehicleModelProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<VehicleModelProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as VehicleModelWhereInput;
    const keys = Object.keys(filter) as VehicleModelKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        currentFilter.type &&
        currentFilter.filterModel === 'number' &&
      ...
      if (
        ...
        where[item as keyof Omit<VehicleModelProps, '__typename'>] = {
    ...
    setRowData(data.getAllVehicleModel ?? []);
  };

  const rowClassRules: RowClassRules<VehicleModelProps> = {
  ...
  useEffect(() => {
    if (dataVehicleModels?.getAllVehicleModel ?? []) {
      setRowData(dataVehicleModels?.getAllVehicleModel ?? []);
    }
  }, [isLoading]);
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateVehicleModels}
                      isLoading={isFetchingUpdateVehicleModels}
                      isDisabled={isFetchingUpdateVehicleModels}
                    >
                      Salvar alterações
                    </Button>
                  </div>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingVehicleModels,
                    'opacity-0 pointer-events-none': !isFetchingVehicleModels,
                  },
                )}
              >
                <PreLoader />
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteVehicleModels}
                              isLoading={isFetchingDeleteVehicleModels}
                              isDisabled={isFetchingDeleteVehicleModels}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataVehicleModels.totalVehicleModels / pageSize,
                )} página${
                  Math.ceil(dataVehicleModels.totalVehicleModels / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataVehicleModels?.totalVehicleModels} registro${
                  dataVehicleModels?.totalVehicleModels > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingVehicleModels}
