...
import {
  useGetAllCarrierCompanyQuery,
  type CarrierCompanyWhereInput,
  type CarrierCompanyUpdateManyInput,
  useDeleteManyCarrierCompaniesMutation,
  useUpdateManyCarrierCompaniesMutation,
...
import {
  ...
  type CarrierProps,
  type EditRowProps,
  type CarrierKeyProps,
  type DataTableCarriersProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableCarriers = ({ ...props }: DataTableCarriersProps) => {
  const [isFetchingCarriers, setIsFetchingCarriers] = useState(false);
  const [isFetchingUpdateCarriers, setIsFetchingUpdateCarriers] =
    useState(false);
  const [isFetchingDeleteCarriers, setIsFetchingDeleteCarriers] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<CarrierProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<CarrierProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyCarrierCompaniesMutation();
  const [deleteCarriers] = useDeleteManyCarrierCompaniesMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<CarrierProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as CarrierKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: CarrierProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<CarrierCompanyUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<CarrierProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    const { data: result } = await refetch({
      ...
      where: { rntrc: { contains: search } },
    });

    setRowData(result.getAllCarrierCompany ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllCarrierCompany ?? []);

        return {
          ...fetchMoreResult,
          Carriers: [
            ...(previous.getAllCarrierCompany ?? []),
            ...(fetchMoreResult.getAllCarrierCompany ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateCarriers = async () => {
    setIsFetchingUpdateCarriers(true);

    try {
      const { data } = await update({
        variables: {
          updateManyCarrierCompanies: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyCarrierCompanies &&
        data.updateManyCarrierCompanies.length > 1;

      toast.success(
        `Transportadora${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a transportadora!', {
      ...
      setIsFetchingUpdateCarriers(false);
    }
  };

  const handleDeleteCarriers = async () => {
    setIsFetchingDeleteCarriers(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteCarriers({
        variables: { deleteManyCarrierCompanies: ids },
      });

      const isPlural =
        data?.deleteManyCarrierCompanies &&
        data?.deleteManyCarrierCompanies.length > 1;

      toast.success(
        `Transportadora${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a transportadora!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteCarriers(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<CarrierProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<CarrierProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as CarrierCompanyWhereInput;
    const keys = Object.keys(filter) as CarrierKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<CarrierProps, '__typename' | 'legalPersonId'>
        ] = {
    ...
    setRowData(data.getAllCarrierCompany ?? []);
  };

  const rowClassRules: RowClassRules<CarrierProps> = {
  ...
  useEffect(() => {
    if (dataCarriers?.getAllCarrierCompany ?? []) {
      setRowData(dataCarriers?.getAllCarrierCompany ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateCarriers}
                      isLoading={isFetchingUpdateCarriers}
                      isDisabled={isFetchingUpdateCarriers}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/carrier-companies/new">
              <Button variant="label">Adicionar Transportadora</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingCarriers,
                    'opacity-0 pointer-events-none': !isFetchingCarriers,
                  },
                )}
              >
                <PreLoader />
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteCarriers}
                              isLoading={isFetchingDeleteCarriers}
                              isDisabled={isFetchingDeleteCarriers}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataCarriers.totalCarrierCompanies / pageSize,
                )} página${
                  Math.ceil(dataCarriers.totalCarrierCompanies / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataCarriers?.totalCarrierCompanies} registro${
                  dataCarriers?.totalCarrierCompanies > 1 ? 's' : ''
                }`}
              </span>
