...
import {
  useGetAllLegalContractQuery,
  type LegalContractWhereInput,
  type LegalContractUpdateManyInput,
  useUpdateManyLegalContractMutation,
  useDeleteManyLegalContractMutation,
...
import {
  type FilterProps,
  type EditRowProps,
  type LegalContractProps,
  type LegalContractKeyProps,
  type DataTableLegalContractsProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableLegalContracts = ({
  ...props
}: DataTableLegalContractsProps) => {
  const [isFetchingLegalContracts, setIsFetchingLegalContracts] =
    useState(false);
  const [isFetchingUpdateLegalContracts, setIsFetchingUpdateLegalContracts] =
    useState(false);
  const [isFetchingDeleteLegalContracts, setIsFetchingDeleteLegalContracts] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<LegalContractProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<LegalContractProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyLegalContractMutation();
  const [deleteLegalContracts] = useDeleteManyLegalContractMutation();

  const {
    refetch,
    fetchMore,
    loading: isLoading,
    data: dataLegalContracts,
  } = useGetAllLegalContractQuery();
  ...
  const valueSetter = (parameters: ValueSetterParams<LegalContractProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as LegalContractKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: LegalContractProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<LegalContractUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<LegalContractProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        headerName: 'Condições de Entrega',
        filterParams: filterParametersText,
      },
      {
        filter: true,
      ...
      },
      {
        filter: true,
        editable: true,
        field: 'legal_client_id',
        headerName: 'Cliente Jurídico',
        filterParams: filterParametersText,
      },
      {
        filter: true,
        editable: true,
        field: 'carrier_company_id',
        filterParams: filterParametersText,
        headerName: 'Empresa Transportadora',
      },
      {
        filter: true,
        editable: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingLegalContracts(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { contract_number: { contains: search } },
    });

    setRowData(result.getAllLegalContract ?? []);
    setIsFetchingLegalContracts(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllLegalContract ?? []);

        return {
          ...fetchMoreResult,
          LegalContracts: [
            ...(previous.getAllLegalContract ?? []),
            ...(fetchMoreResult.getAllLegalContract ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateLegalContracts = async () => {
    setIsFetchingUpdateLegalContracts(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyLegalContract &&
        data.updateManyLegalContract.length > 1;

      toast.success(
        `Contrato${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Contrato!', {
      ...
      setIsFetchingUpdateLegalContracts(false);
    }
  };

  const handleDeleteLegalContracts = async () => {
    setIsFetchingDeleteLegalContracts(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteLegalContracts({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyLegalContract &&
        data?.deleteManyLegalContract.length > 1;

      toast.success(
        `Contrato${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Contrato!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteLegalContracts(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<LegalContractProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<LegalContractProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as LegalContractWhereInput;
    const keys = Object.keys(filter) as LegalContractKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<LegalContractProps, '__typename' | 'legalPersonId'>
        ] = {
    ...
    setRowData(data.getAllLegalContract ?? []);
  };

  const rowClassRules: RowClassRules<LegalContractProps> = {
  ...
  useEffect(() => {
    if (dataLegalContracts?.getAllLegalContract ?? []) {
      setRowData(dataLegalContracts?.getAllLegalContract ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateLegalContracts}
                      isLoading={isFetchingUpdateLegalContracts}
                      isDisabled={isFetchingUpdateLegalContracts}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/legal-contracts/new">
              <Button variant="label">Adicionar Contrato</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingLegalContracts,
                    'opacity-0 pointer-events-none': !isFetchingLegalContracts,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/legal-contracts/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteLegalContracts}
                              isLoading={isFetchingDeleteLegalContracts}
                              isDisabled={isFetchingDeleteLegalContracts}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataLegalContracts.countLegalContract / pageSize,
                )} página${
                  Math.ceil(dataLegalContracts.countLegalContract / pageSize) >
                  1
                    ? 's'
                    : ''
                } e ${dataLegalContracts?.countLegalContract} registro${
                  dataLegalContracts?.countLegalContract > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingLegalContracts}
