...
import {
  ...
  useUpdateManyOrderProcessingMutation,
  useDeleteManyOrderProcessingMutation,
...
import {
  ...
  type OrderProcessingKeyProps,
  type DataTableOrderProcessingProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableOrderProcessing = ({
  ...props
}: DataTableOrderProcessingProps) => {
  const [isFetchingOrderProcessing, setIsFetchingOrderProcessing] =
    useState(false);
  const [isFetchingUpdateOrderProcessing, setIsFetchingUpdateOrderProcessing] =
    useState(false);
  const [isFetchingDeleteOrderProcessing, setIsFetchingDeleteOrderProcessing] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<OrderProcessingProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<OrderProcessingProps[]>([]);
  ...
  const valueSetter = (parameters: ValueSetterParams<OrderProcessingProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as OrderProcessingKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: OrderProcessingProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<OrderProcessingUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<OrderProcessingProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        headerName: 'Distancia Total',
        filterParams: filterParametersNumber,
      },
      {
        filter: true,
        editable: true,
        field: 'total_spend_liters',
        filterParams: filterParametersNumber,
        headerName: 'Total de Litros Gastos',
      },
      {
        filter: true,
        editable: true,
        field: 'total_spending_money',
        headerName: 'Total de Gastos',
        filterParams: filterParametersNumber,
      },
      {
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingOrderProcessing(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { order_processing_number: { contains: search } },
    });

    setRowData(result.getAllOrderProcessing ?? []);
    setIsFetchingOrderProcessing(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllOrderProcessing ?? []);

        return {
          ...fetchMoreResult,
          OrderProcessing: [
            ...(previous.getAllOrderProcessing ?? []),
            ...(fetchMoreResult.getAllOrderProcessing ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateOrderProcessing = async () => {
    setIsFetchingUpdateOrderProcessing(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyOrderProcessing &&
        data?.updateManyOrderProcessing.length > 1;

      toast.success(
        `Processamento${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Processamento!', {
      ...
      setIsFetchingUpdateOrderProcessing(false);
    }
  };

  const handleDeleteOrderProcessing = async () => {
    setIsFetchingDeleteOrderProcessing(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteOrderProcessing({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyOrderProcessing &&
        data?.deleteManyOrderProcessing.length > 1;

      toast.success(
        `Processamento${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Processamento!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteOrderProcessing(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<OrderProcessingProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<OrderProcessingProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as OrderProcessingWhereInput;
    const keys = Object.keys(filter) as OrderProcessingKeyProps[];
    ...
    setRowData(data.getAllOrderProcessing ?? []);
  };

  const rowClassRules: RowClassRules<OrderProcessingProps> = {
  ...
  useEffect(() => {
    if (dataOrderProcessing?.getAllOrderProcessing ?? []) {
      setRowData(dataOrderProcessing?.getAllOrderProcessing ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateOrderProcessing}
                      isLoading={isFetchingUpdateOrderProcessing}
                      isDisabled={isFetchingUpdateOrderProcessing}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/order-processing/new">
              <Button variant="label">Adicionar Processamento</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingOrderProcessing,
                    'opacity-0 pointer-events-none': !isFetchingOrderProcessing,
                  },
                )}
              >
                <PreLoader />
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataOrderProcessing.countOrderProcessing / pageSize,
                )} página${
                  Math.ceil(
                    dataOrderProcessing.countOrderProcessing / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataOrderProcessing?.countOrderProcessing} registro${
                  dataOrderProcessing?.countOrderProcessing > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingOrderProcessing}
