...
import {
  ...
  useDeleteManyVehicleTypesMutation,
  useUpdateManyVehicleTypesMutation,
...
import {
  ...
  type VehicleTypeKeyProps,
  type DataTableVehicleTypesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableVehicleTypes = ({
  ...props
}: DataTableVehicleTypesProps) => {
  const [isFetchingVehicleTypes, setIsFetchingVehicleTypes] = useState(false);
  const [isFetchingUpdateVehicleTypes, setIsFetchingUpdateVehicleTypes] =
    useState(false);
  const [isFetchingDeleteVehicleTypes, setIsFetchingDeleteVehicleTypes] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<VehicleTypeProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<VehicleTypeProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyVehicleTypesMutation();
  const [deleteVehicleTypes] = useDeleteManyVehicleTypesMutation();
  ...
  const filterParametersBool: INumberFilterParams = {
    filterPlaceholder: 'Filtro',
    filterOptions: [
      'empty',
      {
        ...
        displayKey: 'equals',
        displayName: 'Ativo',
        predicate: (_, cellValue: boolean) => cellValue,
      },
      {
        numberOfInputs: 0,
        displayKey: 'notEquals',
  ...
  const valueSetter = (parameters: ValueSetterParams<VehicleTypeProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as VehicleTypeKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: VehicleTypeProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<VehicleTypeUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<VehicleTypeProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        filterParams: filterParametersBool,
      },
      {
        filter: true,
        editable: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    setRowData(result.getAllVehicleTypes ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllVehicleTypes ?? []);

        return {
          ...fetchMoreResult,
          VehicleTypes: [
            ...(previous.getAllVehicleTypes ?? []),
            ...(fetchMoreResult.getAllVehicleTypes ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateVehicleTypes = async () => {
    setIsFetchingUpdateVehicleTypes(true);

    try {
      const { data } = await update({
        variables: {
          updateManyVehicleTypes: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyVehicleTypes && data.updateManyVehicleTypes.length > 1;

      toast.success(
        `Tipo de Veículo${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Tipo de Veículo!', {
      ...
      setIsFetchingUpdateVehicleTypes(false);
    }
  };

  const handleDeleteVehicleTypes = async () => {
    setIsFetchingDeleteVehicleTypes(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteVehicleTypes({
        variables: { deleteManyVehicleTypes: ids },
      });

      const isPlural =
        data?.deleteManyVehicleTypes && data?.deleteManyVehicleTypes.length > 1;

      toast.success(
        `Tipo de Veículo${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Tipo de Veículo!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteVehicleTypes(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<VehicleTypeProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<VehicleTypeProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as VehicleTypeWhereInput;
    const keys = Object.keys(filter) as VehicleTypeKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[item as keyof Omit<VehicleTypeProps, 'bodyWork' | '__typename'>] =
          {
    ...
    setRowData(data.getAllVehicleTypes ?? []);
  };

  const rowClassRules: RowClassRules<VehicleTypeProps> = {
  ...
  useEffect(() => {
    if (dataVehicleTypes?.getAllVehicleTypes ?? []) {
      setRowData(dataVehicleTypes?.getAllVehicleTypes ?? []);
    }
  }, [isLoading]);
        ...
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      isLoading={isFetchingUpdateVehicleTypes}
                      isDisabled={isFetchingUpdateVehicleTypes}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/vehicle-types/new">
              <Button variant="label">Adicionar Tipo de Veículo</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingVehicleTypes,
                    'opacity-0 pointer-events-none': !isFetchingVehicleTypes,
                  },
                )}
              >
                <PreLoader />
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteVehicleTypes}
                              isDisabled={isFetchingDeleteVehicleTypes}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataVehicleTypes.totalVehicleTypes / pageSize,
                )} página${
                  Math.ceil(dataVehicleTypes.totalVehicleTypes / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataVehicleTypes?.totalVehicleTypes} registro${
                  dataVehicleTypes?.totalVehicleTypes > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingVehicleTypes}
