...
import {
  useGetAllSenderQuery,
  type SenderWhereInput,
  type SenderUpdateManyInput,
  useDeleteManySendersMutation,
  useUpdateManySendersMutation,
...
import {
  ...
  type EditRowProps,
  type SenderKeyProps,
  type DataTableSendersProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableSenders = ({ ...props }: DataTableSendersProps) => {
  const [isFetchingSenders, setIsFetchingSenders] = useState(false);
  const [isFetchingUpdateSenders, setIsFetchingUpdateSenders] = useState(false);
  const [isFetchingDeleteSenders, setIsFetchingDeleteSenders] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<SenderProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<SenderProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManySendersMutation();
  const [deleteSenders] = useDeleteManySendersMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<SenderProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as SenderKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: SenderProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<SenderUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<SenderProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    setRowData(result.getAllSender ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllSender ?? []);

        return {
          ...fetchMoreResult,
          Senders: [
            ...(previous.getAllSender ?? []),
            ...(fetchMoreResult.getAllSender ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateSenders = async () => {
    setIsFetchingUpdateSenders(true);

    try {
      const { data } = await update({
        variables: {
          updateManySenders: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManySenders && data.updateManySenders.length > 1;

      toast.success(
        `Remetente${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Remetente!', {
      ...
      setIsFetchingUpdateSenders(false);
    }
  };

  const handleDeleteSenders = async () => {
    setIsFetchingDeleteSenders(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteSenders({
        variables: { deleteManySenders: ids },
      });

      const isPlural =
        data?.deleteManySenders && data?.deleteManySenders.length > 1;

      toast.success(
        `Remetente${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Remetente!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteSenders(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<SenderProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<SenderProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as SenderWhereInput;
    const keys = Object.keys(filter) as SenderKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[item as keyof Omit<SenderProps, '__typename' | 'legalPersonId'>] =
          {
    ...
    setRowData(data.getAllSender ?? []);
  };

  const rowClassRules: RowClassRules<SenderProps> = {
  ...
  useEffect(() => {
    if (dataSenders?.getAllSender ?? []) {
      setRowData(dataSenders?.getAllSender ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateSenders}
                      isLoading={isFetchingUpdateSenders}
                      isDisabled={isFetchingUpdateSenders}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/senders/new">
              <Button variant="label">Adicionar Remetente</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingSenders,
                    'opacity-0 pointer-events-none': !isFetchingSenders,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/senders/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteSenders}
                              isDisabled={isFetchingDeleteSenders}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataSenders.totalSenders / pageSize,
                )} página${
                  Math.ceil(dataSenders.totalSenders / pageSize) > 1 ? 's' : ''
                } e ${dataSenders?.totalSenders} registro${
                  dataSenders?.totalSenders > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                length={Math.ceil(dataSenders.totalSenders / pageSize)}
