...
import {
  TypesCteEnum,
  useGetAllLegalClientOrderQuery,
  type LegalClientOrderWhereInput,
  useCreateLegalClientCteMutation,
  type LegalClientOrderUpdateManyInput,
  useUpdateManyLegalClientOrderMutation,
  useDeleteManyLegalClientOrderMutation,
...
import { generateLegalCTE } from 'actions/cte/generateLegal';
import {
  type ColDef,
  type RowClassRules,
...
import {
  ...
  type EditRowProps,
  type CTESchemaProps,
  type LegalClientOrderProps,
  type LegalClientOrderKeyProps,
  type DataTableLegalClientOrdersProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableLegalClientOrders = ({
  ...props
}: DataTableLegalClientOrdersProps) => {
  const [isFetchingLegalClientOrders, setIsFetchingLegalClientOrders] =
    useState(false);
  const [
    isFetchingUpdateLegalClientOrders,
    setIsFetchingUpdateLegalClientOrders,
  ] = useState(false);
  const [
    isFetchingDeleteLegalClientOrders,
    setIsFetchingDeleteLegalClientOrders,
  ] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<LegalClientOrderProps[]>([]);
  ...
  const [selectedRows, setSelectedRows] = useState<LegalClientOrderProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyLegalClientOrderMutation();
  const [deleteLegalClientOrders] = useDeleteManyLegalClientOrderMutation();

  const [createCTE] = useCreateLegalClientCteMutation();
  ...
  const handleCreateCTE = async (cte: CTESchemaProps) => {
    ...
    try {
      ...
      if (resultCTE?.createLegalClientCte?.id) {
        const { data: generateCte } = await generateLegalCTE({
          request: {
            cteLegalPersonId: resultCTE?.createLegalClientCte?.id,
          },
        });

        window.open(
          generateCte.generateLegalClientCte.cteUrl,
          '_blank',
          'noopener,noreferrer',
        );
      }
  ...
  const valueSetter = (
    parameters: ValueSetterParams<LegalClientOrderProps>,
  ) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as LegalClientOrderKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: LegalClientOrderProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<LegalClientOrderUpdateManyInput> =
        {
          id,
  ...
  const colDefs = useMemo<ColDef<LegalClientOrderProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingLegalClientOrders(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { legal_contract_id: { contains: search } },
    });

    setRowData(result.getAllLegalClientOrder ?? []);
    setIsFetchingLegalClientOrders(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllLegalClientOrder ?? []);

        return {
          ...fetchMoreResult,
          LegalClientOrders: [
            ...(previous.getAllLegalClientOrder ?? []),
            ...(fetchMoreResult.getAllLegalClientOrder ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateLegalClientOrders = async () => {
    setIsFetchingUpdateLegalClientOrders(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyLegalClientOrder &&
        data?.updateManyLegalClientOrder.length > 1;
      ...
      setIsFetchingUpdateLegalClientOrders(false);
    }
  };

  const handleDeleteLegalClientOrders = async () => {
    setIsFetchingDeleteLegalClientOrders(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteLegalClientOrders({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyLegalClientOrder &&
        data?.deleteManyLegalClientOrder.length > 1;
      ...
      setIsFetchingDeleteLegalClientOrders(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<LegalClientOrderProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<LegalClientOrderProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as LegalClientOrderWhereInput;
    const keys = Object.keys(filter) as LegalClientOrderKeyProps[];
    ...
    setRowData(data.getAllLegalClientOrder ?? []);
  };

  const rowClassRules: RowClassRules<LegalClientOrderProps> = {
  ...
  useEffect(() => {
    if (dataLegalClientOrders?.getAllLegalClientOrder ?? []) {
      setRowData(dataLegalClientOrders?.getAllLegalClientOrder ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateLegalClientOrders}
                      isLoading={isFetchingUpdateLegalClientOrders}
                      isDisabled={isFetchingUpdateLegalClientOrders}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/legal-client-orders/new">
        ...
        {dataLegalClientOrders && !isLoading && (
          <>
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              variant="label"
                              className="min-w-[5.806rem]"
                              onClick={handleDeleteLegalClientOrders}
                              isLoading={isFetchingDeleteLegalClientOrders}
                              isDisabled={isFetchingDeleteLegalClientOrders}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataLegalClientOrders.countLegalClientOrder / pageSize,
                )} página${
                  Math.ceil(
                    dataLegalClientOrders.countLegalClientOrder / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataLegalClientOrders?.countLegalClientOrder} registro${
                  dataLegalClientOrders?.countLegalClientOrder > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingLegalClientOrders}
