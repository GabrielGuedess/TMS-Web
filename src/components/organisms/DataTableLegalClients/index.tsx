...
import {
  ...
  useDeleteManyLegalClientsMutation,
  useUpdateManyLegalClientsMutation,
...
import {
  ...
  type LegalClientKeyProps,
  type DataTableLegalClientsProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableLegalClients = ({
  ...props
}: DataTableLegalClientsProps) => {
  const [isFetchingLegalClients, setIsFetchingLegalClients] = useState(false);
  const [isFetchingUpdateLegalClients, setIsFetchingUpdateLegalClients] =
    useState(false);
  const [isFetchingDeleteLegalClients, setIsFetchingDeleteLegalClients] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<LegalClientProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<LegalClientProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyLegalClientsMutation();
  const [deleteLegalClients] = useDeleteManyLegalClientsMutation();

  const {
    refetch,
    fetchMore,
    loading: isLoading,
    data: dataLegalClients,
  } = useGetAllLegalClientQuery();
  ...
  const valueSetter = (parameters: ValueSetterParams<LegalClientProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as LegalClientKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: LegalClientProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<LegalClientUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<LegalClientProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingLegalClients(true);
    setSelectedRows([]);
    ...
    setRowData(result.getAllLegalClient ?? []);
    setIsFetchingLegalClients(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllLegalClient ?? []);

        return {
          ...fetchMoreResult,
          LegalClients: [
            ...(previous.getAllLegalClient ?? []),
            ...(fetchMoreResult.getAllLegalClient ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateLegalClients = async () => {
    setIsFetchingUpdateLegalClients(true);

    try {
      const { data } = await update({
        variables: {
          updateManyLegalClients: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyLegalClients && data.updateManyLegalClients.length > 1;

      toast.success(
        `Clientes Jurídico${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Cliente Jurídico!', {
      ...
      setIsFetchingUpdateLegalClients(false);
    }
  };

  const handleDeleteLegalClients = async () => {
    setIsFetchingDeleteLegalClients(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteLegalClients({
        variables: { deleteManyLegalClients: ids },
      });

      const isPlural =
        data?.deleteManyLegalClients && data?.deleteManyLegalClients.length > 1;

      toast.success(
        `Clientes Jurídico${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Cliente Jurídico!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteLegalClients(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<LegalClientProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<LegalClientProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as LegalClientWhereInput;
    const keys = Object.keys(filter) as LegalClientKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<
            LegalClientProps,
            '__typename' | 'updated_at' | 'created_at' | 'legalPersonId'
          >
        ] = {
    ...
    setRowData(data.getAllLegalClient ?? []);
  };

  const rowClassRules: RowClassRules<LegalClientProps> = {
  ...
  useEffect(() => {
    if (dataLegalClients?.getAllLegalClient ?? []) {
      setRowData(dataLegalClients?.getAllLegalClient ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateLegalClients}
                      isLoading={isFetchingUpdateLegalClients}
                      isDisabled={isFetchingUpdateLegalClients}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/legal-clients/new">
              <Button variant="label">Adicionar Cliente Jurídico</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingLegalClients,
                    'opacity-0 pointer-events-none': !isFetchingLegalClients,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/legal-clients/${selectedRows[0].LegalPerson.cnpj}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteLegalClients}
                              isLoading={isFetchingDeleteLegalClients}
                              isDisabled={isFetchingDeleteLegalClients}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataLegalClients.totalLegalClients / pageSize,
                )} página${
                  Math.ceil(dataLegalClients.totalLegalClients / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataLegalClients?.totalLegalClients} registro${
                  dataLegalClients?.totalLegalClients > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingLegalClients}
