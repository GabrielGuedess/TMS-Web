...
import {
  ...
  useDeleteManyMaintenanceMutation,
...
import {
  ...
  type MaintenanceKeyProps,
  type DataTableMaintenancesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableMaintenances = ({
  ...props
}: DataTableMaintenancesProps) => {
  const [isFetchingMaintenances, setIsFetchingMaintenances] = useState(false);
  const [isFetchingUpdateMaintenances, setIsFetchingUpdateMaintenances] =
    useState(false);
  const [isFetchingDeleteMaintenances, setIsFetchingDeleteMaintenances] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<MaintenanceProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<MaintenanceProps[]>([]);
  ...
  const [deleteMaintenances] = useDeleteManyMaintenanceMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<MaintenanceProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as MaintenanceKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: MaintenanceProps = {
        ...parameters.data,
  ...
  const colDefs = useMemo<ColDef<MaintenanceProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        filterParams: filterParametersText,
        headerName: 'Empresa de Manutenção',
      },
      {
        filter: true,
        editable: true,
        field: 'type_of_maintenance_id',
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingMaintenances(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { vehicle_id: { contains: search } },
    });

    setRowData(result.getAllMaintenance ?? []);
    setIsFetchingMaintenances(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllMaintenance ?? []);

        return {
          ...fetchMoreResult,
          Maintenances: [
            ...(previous.getAllMaintenance ?? []),
            ...(fetchMoreResult.getAllMaintenance ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateMaintenances = async () => {
    setIsFetchingUpdateMaintenances(true);
    ...
    try {
      ...
      toast.success(
        `Manutenç${isPlural ? 'ões' : 'ão'} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Manutenção!', {
      ...
      setIsFetchingUpdateMaintenances(false);
    }
  };

  const handleDeleteMaintenances = async () => {
    setIsFetchingDeleteMaintenances(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteMaintenances({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyMaintenance && data?.deleteManyMaintenance.length > 1;

      toast.success(
        `Manutenç${isPlural ? 'ões' : 'ão'} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      setIsFetchingDeleteMaintenances(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<MaintenanceProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<MaintenanceProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as MaintenanceWhereInput;
    const keys = Object.keys(filter) as MaintenanceKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[item as keyof Omit<MaintenanceProps, '__typename'>] = {
    ...
    setRowData(data.getAllMaintenance ?? []);
  };

  const rowClassRules: RowClassRules<MaintenanceProps> = {
  ...
  useEffect(() => {
    if (dataMaintenances?.getAllMaintenance ?? []) {
      setRowData(dataMaintenances?.getAllMaintenance ?? []);
    }
  }, [isLoading]);
        ...
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateMaintenances}
                      isLoading={isFetchingUpdateMaintenances}
                      isDisabled={isFetchingUpdateMaintenances}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/maintenances/new">
              <Button variant="label">Adicionar Manutenção</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingMaintenances,
                    'opacity-0 pointer-events-none': !isFetchingMaintenances,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/maintenances/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteMaintenances}
                              isDisabled={isFetchingDeleteMaintenances}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataMaintenances.countMaintenance / pageSize,
                )} página${
                  Math.ceil(dataMaintenances.countMaintenance / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataMaintenances?.countMaintenance} registro${
                  dataMaintenances?.countMaintenance > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingMaintenances}
                onChangeIndex={handleFetchMorePage}
                onChangeWithEdited={() => setIsOpenDiscardChanges(true)}
                length={Math.ceil(dataMaintenances.countMaintenance / pageSize)}
              />
            </div>
          </>
        )}
