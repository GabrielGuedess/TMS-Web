...
import {
  useGetAllPhysicalCustomerQuery,
  type PhysicalCustomerWhereInput,
  type PhysicalCustomerUpdateManyInput,
  useDeleteManyPhysicalCustomersMutation,
  useUpdateManyPhysicalCustomersMutation,
...
import {
  type FilterProps,
  type EditRowProps,
  type PhysicalCustomerProps,
  type PhysicalCustomerKeyProps,
  type DataTablePhysicalCustomersProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTablePhysicalCustomers = ({
  ...props
}: DataTablePhysicalCustomersProps) => {
  const [isFetchingPhysicalCustomers, setIsFetchingPhysicalCustomers] =
    useState(false);
  const [
    isFetchingUpdatePhysicalCustomers,
    setIsFetchingUpdatePhysicalCustomers,
  ] = useState(false);
  const [
    isFetchingDeletePhysicalCustomers,
    setIsFetchingDeletePhysicalCustomers,
  ] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<PhysicalCustomerProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<PhysicalCustomerProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyPhysicalCustomersMutation();
  const [deletePhysicalCustomers] = useDeleteManyPhysicalCustomersMutation();

  const {
    refetch,
    fetchMore,
    loading: isLoading,
    data: dataPhysicalCustomers,
  } = useGetAllPhysicalCustomerQuery();
  ...
  const valueSetter = (
    parameters: ValueSetterParams<PhysicalCustomerProps>,
  ) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as PhysicalCustomerKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: PhysicalCustomerProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<PhysicalCustomerUpdateManyInput> =
        {
          id,
  ...
  const colDefs = useMemo<ColDef<PhysicalCustomerProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingPhysicalCustomers(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { branch: { contains: search } },
    });

    setRowData(result.getAllPhysicalCustomer ?? []);
    setIsFetchingPhysicalCustomers(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllPhysicalCustomer ?? []);

        return {
          ...fetchMoreResult,
          PhysicalCustomers: [
            ...(previous.getAllPhysicalCustomer ?? []),
            ...(fetchMoreResult.getAllPhysicalCustomer ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdatePhysicalCustomers = async () => {
    setIsFetchingUpdatePhysicalCustomers(true);

    try {
      const { data } = await update({
        variables: {
          updateManyPhysicalCustomers: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyPhysicalCustomers &&
        data.updateManyPhysicalCustomers.length > 1;

      toast.success(
        `Clientes Físico${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Cliente Físico!', {
      ...
      setIsFetchingUpdatePhysicalCustomers(false);
    }
  };

  const handleDeletePhysicalCustomers = async () => {
    setIsFetchingDeletePhysicalCustomers(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deletePhysicalCustomers({
        variables: { deleteManyPhysicalCustomers: ids },
      });

      const isPlural =
        data?.deleteManyPhysicalCustomers &&
        data?.deleteManyPhysicalCustomers.length > 1;

      toast.success(
        `Clientes Físico${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Cliente Físico!', {
          description: error.message,
        });
      ...
      setIsFetchingDeletePhysicalCustomers(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<PhysicalCustomerProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<PhysicalCustomerProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as PhysicalCustomerWhereInput;
    const keys = Object.keys(filter) as PhysicalCustomerKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<
            PhysicalCustomerProps,
            '__typename' | 'legalPersonId'
          >
        ] = {
    ...
    setRowData(data.getAllPhysicalCustomer ?? []);
  };

  const rowClassRules: RowClassRules<PhysicalCustomerProps> = {
  ...
  useEffect(() => {
    if (dataPhysicalCustomers?.getAllPhysicalCustomer ?? []) {
      setRowData(dataPhysicalCustomers?.getAllPhysicalCustomer ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdatePhysicalCustomers}
                      isLoading={isFetchingUpdatePhysicalCustomers}
                      isDisabled={isFetchingUpdatePhysicalCustomers}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/physical-customers/new">
              <Button variant="label">Adicionar Cliente Físico</Button>
            </Link>
        ...
        {dataPhysicalCustomers && !isLoading && (
          <>
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/physical-customers/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeletePhysicalCustomers}
                              isLoading={isFetchingDeletePhysicalCustomers}
                              isDisabled={isFetchingDeletePhysicalCustomers}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataPhysicalCustomers.totalPhysicalCustomers / pageSize,
                )} página${
                  Math.ceil(
                    dataPhysicalCustomers.totalPhysicalCustomers / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataPhysicalCustomers?.totalPhysicalCustomers} registro${
                  dataPhysicalCustomers?.totalPhysicalCustomers > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingPhysicalCustomers}
