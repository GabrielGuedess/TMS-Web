...
import {
  ...
  useDeleteManyOwnDriversMutation,
  useUpdateManyOwnDriversMutation,
...
import {
  ...
  type EditRowProps,
  type OwnDriverProps,
  type OwnDriverKeyProps,
  type DataTableOwnDriversProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableOwnDrivers = ({ ...props }: DataTableOwnDriversProps) => {
  const [isFetchingOwnDrivers, setIsFetchingOwnDrivers] = useState(false);
  const [isFetchingUpdateOwnDrivers, setIsFetchingUpdateOwnDrivers] =
    useState(false);
  const [isFetchingDeleteOwnDrivers, setIsFetchingDeleteOwnDrivers] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<OwnDriverProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<OwnDriverProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyOwnDriversMutation();
  const [deleteOwnDrivers] = useDeleteManyOwnDriversMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<OwnDriverProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as OwnDriverKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: OwnDriverProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<OwnDriverUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<OwnDriverProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      {
        initialWidth: 260,
        field: 'cnh_expiration',
        filter: 'agDateColumnFilter',
        filterParams: filterParametersDate,
        headerName: 'Data de Expiração da CNH',
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingOwnDrivers(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { cnh: { contains: search } },
    });

    setRowData(result.getAllOwnDriver ?? []);
    setIsFetchingOwnDrivers(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllOwnDriver ?? []);

        return {
          ...fetchMoreResult,
          OwnDrivers: [
            ...(previous.getAllOwnDriver ?? []),
            ...(fetchMoreResult.getAllOwnDriver ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateOwnDrivers = async () => {
    setIsFetchingUpdateOwnDrivers(true);

    try {
      const { data } = await update({
        variables: {
          updateManyOwnDrivers: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyOwnDrivers && data.updateManyOwnDrivers.length > 1;

      toast.success(
        `Motorista Próprio${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Motorista Próprio!', {
      ...
      setIsFetchingUpdateOwnDrivers(false);
    }
  };

  const handleDeleteOwnDrivers = async () => {
    setIsFetchingDeleteOwnDrivers(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteOwnDrivers({
        variables: { deleteManOwnDrivers: ids },
      });

      const isPlural =
        data?.deleteManyOwnDrivers && data?.deleteManyOwnDrivers.length > 1;

      toast.success(
        `Motorista Próprio${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Motorista Próprio!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteOwnDrivers(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<OwnDriverProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<OwnDriverProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as OwnDriverWhereInput;
    const keys = Object.keys(filter) as OwnDriverKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<OwnDriverProps, '__typename' | 'legalPersonId'>
        ] = {
    ...
    setRowData(data.getAllOwnDriver ?? []);
  };

  const rowClassRules: RowClassRules<OwnDriverProps> = {
  ...
  useEffect(() => {
    if (dataOwnDrivers?.getAllOwnDriver ?? []) {
      setRowData(dataOwnDrivers?.getAllOwnDriver ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateOwnDrivers}
                      isLoading={isFetchingUpdateOwnDrivers}
                      isDisabled={isFetchingUpdateOwnDrivers}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/own-drivers/new">
              <Button variant="label">Adicionar Motorista Próprio</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingOwnDrivers,
                    'opacity-0 pointer-events-none': !isFetchingOwnDrivers,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/own-drivers/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteOwnDrivers}
                              isLoading={isFetchingDeleteOwnDrivers}
                              isDisabled={isFetchingDeleteOwnDrivers}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataOwnDrivers.totalOwnDrivers / pageSize,
                )} página${
                  Math.ceil(dataOwnDrivers.totalOwnDrivers / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataOwnDrivers?.totalOwnDrivers} registro${
                  dataOwnDrivers?.totalOwnDrivers > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingOwnDrivers}
                onChangeIndex={handleFetchMorePage}
                onChangeWithEdited={() => setIsOpenDiscardChanges(true)}
                length={Math.ceil(dataOwnDrivers.totalOwnDrivers / pageSize)}
              />
            </div>
          </>
        )}
