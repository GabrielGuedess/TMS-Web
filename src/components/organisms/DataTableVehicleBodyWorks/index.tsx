...
import {
  useGetAllVehicleBodyworkQuery,
  type VehicleBodyworkWhereInput,
  type VehicleBodyworkUpdateManyInput,
  useUpdateManyVehicleBodyworksMutation,
  useDeleteManyVehicleBodyworksMutation,
...
import {
  type FilterProps,
  type EditRowProps,
  type VehicleBodyWorkProps,
  type VehicleBodyWorkKeyProps,
  type DataTableVehicleBodyWorksProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableVehicleBodyWorks = ({
  ...props
}: DataTableVehicleBodyWorksProps) => {
  const [isFetchingVehicleBodyWorks, setIsFetchingVehicleBodyWorks] =
    useState(false);
  const [
    isFetchingUpdateVehicleBodyWorks,
    setIsFetchingUpdateVehicleBodyWorks,
  ] = useState(false);
  const [
    isFetchingDeleteVehicleBodyWorks,
    setIsFetchingDeleteVehicleBodyWorks,
  ] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<VehicleBodyWorkProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<VehicleBodyWorkProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyVehicleBodyworksMutation();
  const [deleteVehicleBodyWorks] = useDeleteManyVehicleBodyworksMutation();

  const {
    refetch,
    fetchMore,
    loading: isLoading,
    data: dataVehicleBodyWorks,
  } = useGetAllVehicleBodyworkQuery();
  ...
  const valueSetter = (parameters: ValueSetterParams<VehicleBodyWorkProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as VehicleBodyWorkKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: VehicleBodyWorkProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<VehicleBodyworkUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<VehicleBodyWorkProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingVehicleBodyWorks(true);
    setSelectedRows([]);
    ...
    setRowData(result.getAllVehicleBodywork ?? []);
    setIsFetchingVehicleBodyWorks(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllVehicleBodywork ?? []);

        return {
          ...fetchMoreResult,
          VehicleBodyWorks: [
            ...(previous.getAllVehicleBodywork ?? []),
            ...(fetchMoreResult.getAllVehicleBodywork ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateVehicleBodyWorks = async () => {
    setIsFetchingUpdateVehicleBodyWorks(true);

    try {
      const { data } = await update({
        variables: {
          updateManyVehicleBodyworks: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyVehicleBodyworks &&
        data.updateManyVehicleBodyworks.length > 1;

      toast.success(
        `Carroceria de Veículo${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Carroceria de Veículo!', {
      ...
      setIsFetchingUpdateVehicleBodyWorks(false);
    }
  };

  const handleDeleteVehicleBodyWorks = async () => {
    setIsFetchingDeleteVehicleBodyWorks(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteVehicleBodyWorks({
        variables: { deleteManyVehicleBodyworks: ids },
      });

      const isPlural =
        data?.deleteManyVehicleBodyworks &&
        data?.deleteManyVehicleBodyworks.length > 1;

      toast.success(
        `Carroceria de Veículo${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Carroceria de Veículo!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteVehicleBodyWorks(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<VehicleBodyWorkProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<VehicleBodyWorkProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as VehicleBodyworkWhereInput;
    const keys = Object.keys(filter) as VehicleBodyWorkKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        currentFilter.type &&
        currentFilter.filterBodyWork === 'number' &&
      ...
      if (
        ...
        where[item as keyof Omit<VehicleBodyWorkProps, '__typename'>] = {
    ...
    setRowData(data.getAllVehicleBodywork ?? []);
  };

  const rowClassRules: RowClassRules<VehicleBodyWorkProps> = {
  ...
  useEffect(() => {
    if (dataVehicleBodyWorks?.getAllVehicleBodywork ?? []) {
      setRowData(dataVehicleBodyWorks?.getAllVehicleBodywork ?? []);
    }
  }, [isLoading]);
  ...
  return (
    <div className="relative overflow-x-auto shadow-default sm:rounded">
      <div className="flex flex-col">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateVehicleBodyWorks}
                      isLoading={isFetchingUpdateVehicleBodyWorks}
                      isDisabled={isFetchingUpdateVehicleBodyWorks}
                    >
                      Salvar alterações
                    </Button>
                  </div>
        ...
        {dataVehicleBodyWorks && !isLoading && (
          <>
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingVehicleBodyWorks,
                    'opacity-0 pointer-events-none':
                      !isFetchingVehicleBodyWorks,
                  },
                )}
              >
                <PreLoader />
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteVehicleBodyWorks}
                              isLoading={isFetchingDeleteVehicleBodyWorks}
                              isDisabled={isFetchingDeleteVehicleBodyWorks}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataVehicleBodyWorks.totalVehicleBodyworks / pageSize,
                )} página${
                  Math.ceil(
                    dataVehicleBodyWorks.totalVehicleBodyworks / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataVehicleBodyWorks?.totalVehicleBodyworks} registro${
                  dataVehicleBodyWorks?.totalVehicleBodyworks > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingVehicleBodyWorks}
