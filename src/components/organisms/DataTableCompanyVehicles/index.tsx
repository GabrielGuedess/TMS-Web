...
import {
  useGetAllCompanyVehicleQuery,
  type CompanyVehicleWhereInput,
  type CompanyVehicleUpdateManyInput,
  useDeleteManyCompanyVehiclesMutation,
  useUpdateManyCompanyVehiclesMutation,
...
import {
  type FilterProps,
  type EditRowProps,
  type CompanyVehicleProps,
  type CompanyVehicleKeyProps,
  type DataTableCompanyVehiclesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableCompanyVehicles = ({
  ...props
}: DataTableCompanyVehiclesProps) => {
  const [isFetchingCompanyVehicles, setIsFetchingCompanyVehicles] =
    useState(false);
  const [isFetchingUpdateCompanyVehicles, setIsFetchingUpdateCompanyVehicles] =
    useState(false);
  const [isFetchingDeleteCompanyVehicles, setIsFetchingDeleteCompanyVehicles] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<CompanyVehicleProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<CompanyVehicleProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyCompanyVehiclesMutation();
  const [deleteCompanyVehicles] = useDeleteManyCompanyVehiclesMutation();

  const {
    refetch,
    fetchMore,
    loading: isLoading,
    data: dataCompanyVehicles,
  } = useGetAllCompanyVehicleQuery();
  ...
  const valueSetter = (parameters: ValueSetterParams<CompanyVehicleProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as CompanyVehicleKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: CompanyVehicleProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<CompanyVehicleUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<CompanyVehicleProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingCompanyVehicles(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { Vehicle: { plate: { contains: search } } },
    });

    setRowData(result.getAllCompanyVehicle ?? []);
    setIsFetchingCompanyVehicles(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllCompanyVehicle ?? []);

        return {
          ...fetchMoreResult,
          CompanyVehicles: [
            ...(previous.getAllCompanyVehicle ?? []),
            ...(fetchMoreResult.getAllCompanyVehicle ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateCompanyVehicles = async () => {
    setIsFetchingUpdateCompanyVehicles(true);

    try {
      const { data } = await update({
        variables: {
          updateManyCompanyVehicles: editedRows.newData!,
        },
      });

      const isPlural =
        data?.updateManyCompanyVehicles &&
        data.updateManyCompanyVehicles.length > 1;

      toast.success(
        `Veiculo de Empresa${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Veiculo de Empresa!', {
      ...
      setIsFetchingUpdateCompanyVehicles(false);
    }
  };

  const handleDeleteCompanyVehicles = async () => {
    setIsFetchingDeleteCompanyVehicles(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteCompanyVehicles({
        variables: { deleteManyCompanyVehicles: ids },
      });

      const isPlural =
        data?.deleteManyCompanyVehicles &&
        data?.deleteManyCompanyVehicles.length > 1;

      toast.success(
        `Veiculo de Empresa${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Veiculo de Empresa!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteCompanyVehicles(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<CompanyVehicleProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<CompanyVehicleProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as CompanyVehicleWhereInput;
    const keys = Object.keys(filter) as CompanyVehicleKeyProps[];
    ...
    setRowData(data.getAllCompanyVehicle ?? []);
  };

  const rowClassRules: RowClassRules<CompanyVehicleProps> = {
  ...
  useEffect(() => {
    if (dataCompanyVehicles?.getAllCompanyVehicle ?? []) {
      setRowData(dataCompanyVehicles?.getAllCompanyVehicle ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateCompanyVehicles}
                      isLoading={isFetchingUpdateCompanyVehicles}
                      isDisabled={isFetchingUpdateCompanyVehicles}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/company-vehicles/new">
              <Button variant="label">Adicionar Veiculo de Empresa</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingCompanyVehicles,
                    'opacity-0 pointer-events-none': !isFetchingCompanyVehicles,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/company-vehicles/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteCompanyVehicles}
                              isLoading={isFetchingDeleteCompanyVehicles}
                              isDisabled={isFetchingDeleteCompanyVehicles}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataCompanyVehicles.totalCompanyVehicles / pageSize,
                )} página${
                  Math.ceil(
                    dataCompanyVehicles.totalCompanyVehicles / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataCompanyVehicles?.totalCompanyVehicles} registro${
                  dataCompanyVehicles?.totalCompanyVehicles > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingCompanyVehicles}
