...
import { UfEnum, useUpdateIcmsMutation } from 'graphql/generated';
...
import { ICMSGeneralSchema } from './schema';
import {
  type ICMSGeneralProps,
  type ICMSGeneralInputProps,
  type ICMSGeneralOutputProps,
} from './types';

const ICMSGeneralRef: ForwardRefRenderFunction<
  ElementRef<'div'>,
  ICMSGeneralProps
> = ({ data, className, ...props }, ref) => {
  const [isLoading, setIsLoading] = useState(false);

  const [updateICMS] = useUpdateIcmsMutation();

  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<ICMSGeneralInputProps>({
    resolver: zodResolver(ICMSGeneralSchema),
    defaultValues: {
      aliquot: String(data.getICMS.aliquot),
      state_origin: data.getICMS.state_origin,
      recipient_state: data.getICMS.recipient_state,
      effective_date: dayjs(data.getICMS.effective_date as Date).toDate(),
    },
  });

  const handleUpdate = async (newData: ICMSGeneralOutputProps) => {
    setIsLoading(true);

    try {
      await updateICMS({
        variables: {
          updateIcmsId: data.getICMS.id,
          invoiceForLegalClientInput: {
            aliquot: Number(newData.aliquot),
            state_origin: newData.state_origin,
            effective_date: newData.effective_date,
            recipient_state: newData.recipient_state,
          },
        },
      });
        ...
        <div className="flex w-full justify-center sm:justify-end">
          <Button
            onClick={handleSubmit((values: unknown) => {
              const update = values as ICMSGeneralOutputProps;
...
export const ICMSGeneral = forwardRef(ICMSGeneralRef);
