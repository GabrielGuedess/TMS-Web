...
import {
  ...
  useCreateLegalClientOrderMutation,
...
import { createLegalClientOrderSchema } from './schema';
import {
  type CreateLegalClientOrderProps,
  type CreateLegalClientOrderInputProps,
  type CreateLegalClientOrderOutputProps,
} from './types';

const CreateLegalClientOrderRef: ForwardRefRenderFunction<
  ElementRef<'div'>,
  CreateLegalClientOrderProps
> = ({ className, ...props }, ref) => {
  ...
  const [createLegalClientOrder] = useCreateLegalClientOrderMutation();
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreateLegalClientOrderInputProps>({
    resolver: zodResolver(createLegalClientOrderSchema),
    defaultValues: {
      expenses: [],
      carrier: { id: '', description: '' },
      quoteTable: { id: '', description: '' },
      legalContract: { id: '', description: '' },
    },
  });
  ...
  const handleCreate = async (data: CreateLegalClientOrderOutputProps) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao criar Pedido Jurídico!', {
        ...
        <div className="flex w-full justify-center sm:justify-end">
          <Button
            onClick={handleSubmit((values: unknown) => {
              const data = values as CreateLegalClientOrderOutputProps;
...
export const CreateLegalClientOrder = forwardRef(CreateLegalClientOrderRef);
