...
import {
  useGetAllLegalClientQuoteTableQuery,
  type LegalClientQuoteTableWhereInput,
  type LegalClientQuoteTableUpdateManyInput,
  useUpdateManyLegalClientQuoteTableMutation,
  useDeleteManyLegalClientQuoteTableMutation,
...
import {
  type FilterProps,
  type EditRowProps,
  type LegalClientQuoteTableProps,
  type LegalClientQuoteTableKeyProps,
  type DataTableLegalClientQuoteTablesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableLegalClientQuoteTables = ({
  ...props
}: DataTableLegalClientQuoteTablesProps) => {
  const [
    isFetchingLegalClientQuoteTables,
    setIsFetchingLegalClientQuoteTables,
  ] = useState(false);
  const [
    isFetchingUpdateLegalClientQuoteTables,
    setIsFetchingUpdateLegalClientQuoteTables,
  ] = useState(false);
  const [
    isFetchingDeleteLegalClientQuoteTables,
    setIsFetchingDeleteLegalClientQuoteTables,
  ] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<LegalClientQuoteTableProps[]>([]);
  ...
  const [update] = useUpdateManyLegalClientQuoteTableMutation();
  const [deleteLegalClientQuoteTables] =
    useDeleteManyLegalClientQuoteTableMutation();
  ...
  const {
    ...
    fetchMore,
    loading: isLoading,
    data: dataLegalClientQuoteTables,
  } = useGetAllLegalClientQuoteTableQuery();
  ...
  const valueSetter = (
    parameters: ValueSetterParams<LegalClientQuoteTableProps>,
  ) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as LegalClientQuoteTableKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: LegalClientQuoteTableProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<LegalClientQuoteTableUpdateManyInput> =
        {
          id,
  ...
  const colDefs = useMemo<ColDef<LegalClientQuoteTableProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingLegalClientQuoteTables(true);
    setSelectedRows([]);
    ...
    setRowData(result.getAllLegalClientQuoteTable ?? []);
    setIsFetchingLegalClientQuoteTables(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllLegalClientQuoteTable ?? []);

        return {
          ...fetchMoreResult,
          LegalClientQuoteTables: [
            ...(previous.getAllLegalClientQuoteTable ?? []),
            ...(fetchMoreResult.getAllLegalClientQuoteTable ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateLegalClientQuoteTables = async () => {
    setIsFetchingUpdateLegalClientQuoteTables(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyLegalClientQuoteTable &&
        data?.updateManyLegalClientQuoteTable.length > 1;
      ...
      setIsFetchingUpdateLegalClientQuoteTables(false);
    }
  };

  const handleDeleteLegalClientQuoteTables = async () => {
    setIsFetchingDeleteLegalClientQuoteTables(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteLegalClientQuoteTables({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyLegalClientQuoteTable &&
        data?.deleteManyLegalClientQuoteTable.length > 1;
      ...
      setIsFetchingDeleteLegalClientQuoteTables(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<LegalClientQuoteTableProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<LegalClientQuoteTableProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as LegalClientQuoteTableWhereInput;
    const keys = Object.keys(filter) as LegalClientQuoteTableKeyProps[];
    ...
    setRowData(data.getAllLegalClientQuoteTable ?? []);
  };

  const rowClassRules: RowClassRules<LegalClientQuoteTableProps> = {
  ...
  useEffect(() => {
    if (dataLegalClientQuoteTables?.getAllLegalClientQuoteTable ?? []) {
      setRowData(dataLegalClientQuoteTables?.getAllLegalClientQuoteTable ?? []);
    }
  }, [isLoading]);
  ...
  return (
    <div className="relative overflow-x-auto shadow-default sm:rounded">
      <div className="flex flex-col">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateLegalClientQuoteTables}
                      isLoading={isFetchingUpdateLegalClientQuoteTables}
                      isDisabled={isFetchingUpdateLegalClientQuoteTables}
                    >
                      Salvar alterações
                    </Button>
                  </div>
        ...
        {dataLegalClientQuoteTables && !isLoading && (
          <>
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteLegalClientQuoteTables}
                              isLoading={isFetchingDeleteLegalClientQuoteTables}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataLegalClientQuoteTables.countLegalClientQuoteTable /
                    pageSize,
                )} página${
                  Math.ceil(
                    dataLegalClientQuoteTables.countLegalClientQuoteTable /
                    ...
                    ? 's'
                    : ''
                } e ${dataLegalClientQuoteTables?.countLegalClientQuoteTable} registro${
                  dataLegalClientQuoteTables?.countLegalClientQuoteTable > 1
                    ? 's'
                    : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingLegalClientQuoteTables}
