...
import {
  useGetAllFreightExpensesQuery,
  type FreightExpensesWhereInput,
  type FreightExpenseUpdateManyInput,
  useDeleteManyFreightExpensesMutation,
  useUpdateManyFreightExpensesMutation,
...
import {
  ...
  type EditRowProps,
  type ExpenseProps,
  type ExpenseKeyProps,
  type DataTableExpensesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableExpenses = ({ ...props }: DataTableExpensesProps) => {
  const [isFetchingExpenses, setIsFetchingExpenses] = useState(false);
  const [isFetchingUpdateExpenses, setIsFetchingUpdateExpenses] =
    useState(false);
  const [isFetchingDeleteExpenses, setIsFetchingDeleteExpenses] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<ExpenseProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<ExpenseProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyFreightExpensesMutation();
  const [deleteExpenses] = useDeleteManyFreightExpensesMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<ExpenseProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as ExpenseKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: ExpenseProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<FreightExpenseUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<ExpenseProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        filter: true,
        editable: true,
        field: 'legalClientOrderId',
        headerName: 'Pedido Jurídico',
        filterParams: filterParametersText,
      },
      {
        filter: true,
        editable: true,
        headerName: 'Pedido Físico',
        field: 'physicalCustomerOrderId',
        filterParams: filterParametersText,
      },
    ],
    [rowData],
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    const { data: result } = await refetch({
      ...
      where: { expense_name: { contains: search } },
    });

    setRowData(result.getAllFreightExpenses ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllFreightExpenses ?? []);

        return {
          ...fetchMoreResult,
          Expenses: [
            ...(previous.getAllFreightExpenses ?? []),
            ...(fetchMoreResult.getAllFreightExpenses ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateExpenses = async () => {
    setIsFetchingUpdateExpenses(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyFreightExpenses &&
        data?.updateManyFreightExpenses.length > 1;

      toast.success(
        `Despesa${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      setIsFetchingUpdateExpenses(false);
    }
  };

  const handleDeleteExpenses = async () => {
    setIsFetchingDeleteExpenses(true);
    ...
    try {
      ...
      const isPlural =
        data?.deleteManyFreightExpenses &&
        data?.deleteManyFreightExpenses.length > 1;

      toast.success(
        `Despesa${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      setIsFetchingDeleteExpenses(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<ExpenseProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<ExpenseProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as FreightExpensesWhereInput;
    const keys = Object.keys(filter) as ExpenseKeyProps[];
    ...
    setRowData(data.getAllFreightExpenses ?? []);
  };

  const rowClassRules: RowClassRules<ExpenseProps> = {
  ...
  useEffect(() => {
    if (dataExpenses?.getAllFreightExpenses ?? []) {
      setRowData(dataExpenses?.getAllFreightExpenses ?? []);
    }
  }, [isLoading]);
        ...
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      isDisabled={isFetchingUpdateExpenses}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/expenses/new">
              <Button variant="label">Adicionar Despesa</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingExpenses,
                    'opacity-0 pointer-events-none': !isFetchingExpenses,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/expenses/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteExpenses}
                              isDisabled={isFetchingDeleteExpenses}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataExpenses.countFreightExpenses / pageSize,
                )} página${
                  Math.ceil(dataExpenses.countFreightExpenses / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataExpenses?.countFreightExpenses} registro${
                  dataExpenses?.countFreightExpenses > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                length={Math.ceil(dataExpenses.countFreightExpenses / pageSize)}
              />
            </div>
          </>
        )}
