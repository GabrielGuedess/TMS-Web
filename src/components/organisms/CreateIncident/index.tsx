...
import { createIncidentSchema } from './schema';
import {
  type CreateIncidentProps,
  type CreateIncidentInputProps,
  type CreateIncidentOutputProps,
} from './types';

const CreateIncidentRef: ForwardRefRenderFunction<
  ElementRef<'div'>,
  CreateIncidentProps
> = ({ className, ...props }, ref) => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreateIncidentInputProps>({
    resolver: zodResolver(createIncidentSchema),
    defaultValues: {
      description: '',
      date_resolved: dayjs().toDate(),
      date_incident: dayjs().toDate(),
      orderProcess: { id: '', description: '' },
    },
  });
  ...
  const handleCreate = async (data: CreateIncidentOutputProps) => {
    setIsLoading(true);
      ...
      const Incident = await createIncident({
        variables: {
          data: {
            ...
            order_process_id: data.orderProcess.id,
          },
        },
      });
        ...
        <div className="flex w-full justify-center sm:justify-end">
          <Button
            onClick={handleSubmit((values: unknown) => {
              const data = values as CreateIncidentOutputProps;
...
export const CreateIncident = forwardRef(CreateIncidentRef);
