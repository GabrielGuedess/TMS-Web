...
import {
  ...
  type MaintenanceUpdateManyInput,
  useUpdateManyMaintenanceMutation,
  useGetAllMaintenanceCompanyQuery,
  useDeleteManyMaintenanceCompanyMutation,
...
import {
  ...
  type MaintenanceCompanyKeyProps,
  type DataTableMaintenanceCompaniesProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableMaintenanceCompanies = ({
  ...props
}: DataTableMaintenanceCompaniesProps) => {
  const [isFetchingMaintenanceCompanies, setIsFetchingMaintenanceCompanies] =
    useState(false);
  const [
    isFetchingUpdateMaintenanceCompanies,
    setIsFetchingUpdateMaintenanceCompanies,
  ] = useState(false);
  const [
    isFetchingDeleteMaintenanceCompanies,
    setIsFetchingDeleteMaintenanceCompanies,
  ] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<MaintenanceCompanyProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<MaintenanceCompanyProps[]>(
    [],
  );
  ...
  const [update] = useUpdateManyMaintenanceMutation();
  const [deleteMaintenanceCompanies] =
    useDeleteManyMaintenanceCompanyMutation();
  ...
  const valueSetter = (
    parameters: ValueSetterParams<MaintenanceCompanyProps>,
  ) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as MaintenanceCompanyKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: MaintenanceCompanyProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<MaintenanceUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<MaintenanceCompanyProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        filter: true,
        editable: true,
        field: 'specialty_maintenance',
        filterParams: filterParametersText,
        headerName: 'Manutenção especializada',
      },
      {
        filter: true,
        editable: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    setIsFetchingMaintenanceCompanies(true);
    setSelectedRows([]);

    const { data: result } = await refetch({
      limit: pageSize,
      offset: pageSize * page,
      where: { specialty_maintenance: { contains: search } },
    });

    setRowData(result.getAllMaintenanceCompany ?? []);
    setIsFetchingMaintenanceCompanies(false);
  };
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllMaintenanceCompany ?? []);

        return {
          ...fetchMoreResult,
          MaintenanceCompanies: [
            ...(previous.getAllMaintenanceCompany ?? []),
            ...(fetchMoreResult.getAllMaintenanceCompany ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateMaintenanceCompanies = async () => {
    setIsFetchingUpdateMaintenanceCompanies(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyMaintenance && data?.updateManyMaintenance.length > 1;
      toast.success(
        `Empresa${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      setIsFetchingUpdateMaintenanceCompanies(false);
    }
  };

  const handleDeleteMaintenanceCompanies = async () => {
    setIsFetchingDeleteMaintenanceCompanies(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteMaintenanceCompanies({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyMaintenanceCompany &&
        data?.deleteManyMaintenanceCompany.length > 1;

      toast.success(
        `Empresa${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      setIsFetchingDeleteMaintenanceCompanies(false);
    }
  };

  const onSelectionChanged = (
    event: SelectionChangedEvent<MaintenanceCompanyProps>,
  ) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (
    event: FilterChangedEvent<MaintenanceCompanyProps>,
  ) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as MaintenanceWhereInput;
    const keys = Object.keys(filter) as MaintenanceCompanyKeyProps[];
      ...
      if (
        ...
        where[
          item as keyof Omit<
            ...
            '__typename' | 'legal_person_id' | 'specialty_maintenance'
          >
        ] = {
    ...
    setRowData(data.getAllMaintenanceCompany ?? []);
  };

  const rowClassRules: RowClassRules<MaintenanceCompanyProps> = {
  ...
  useEffect(() => {
    if (dataMaintenanceCompanies?.getAllMaintenanceCompany ?? []) {
      setRowData(dataMaintenanceCompanies?.getAllMaintenanceCompany ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateMaintenanceCompanies}
                      isLoading={isFetchingUpdateMaintenanceCompanies}
                      isDisabled={isFetchingUpdateMaintenanceCompanies}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/maintenance-companies/new">
              <Button variant="label">Adicionar Empresa</Button>
            </Link>
        ...
        {dataMaintenanceCompanies && !isLoading && (
          <>
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto':
                      isFetchingMaintenanceCompanies,
                    'opacity-0 pointer-events-none':
                      !isFetchingMaintenanceCompanies,
                  },
                )}
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/maintenance-companies/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              onClick={handleDeleteMaintenanceCompanies}
                              isLoading={isFetchingDeleteMaintenanceCompanies}
                              isDisabled={isFetchingDeleteMaintenanceCompanies}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataMaintenanceCompanies.countMaintenanceCompany / pageSize,
                )} página${
                  Math.ceil(
                    dataMaintenanceCompanies.countMaintenanceCompany / pageSize,
                  ) > 1
                    ? 's'
                    : ''
                } e ${dataMaintenanceCompanies?.countMaintenanceCompany} registro${
                  dataMaintenanceCompanies?.countMaintenanceCompany > 1
                    ? 's'
                    : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                isLoading={isFetchingMaintenanceCompanies}
