...
import {
  useGetAllIcmsQuery,
  type IcmsWhereInput,
  type IcmsUpdateManyInput,
  useDeleteManyIcmsMutation,
  useUpdateManyIcmsMutation,
...
import {
  ...
  type EditRowProps,
  type ICMSKeyProps,
  type DataTableICMSProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableICMS = ({ ...props }: DataTableICMSProps) => {
  const [isFetchingICMS, setIsFetchingICMS] = useState(false);
  const [isFetchingUpdateICMS, setIsFetchingUpdateICMS] = useState(false);
  const [isFetchingDeleteICMS, setIsFetchingDeleteICMS] = useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<ICMSProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<ICMSProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyIcmsMutation();
  const [deleteICMS] = useDeleteManyIcmsMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<ICMSProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as ICMSKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: ICMSProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<IcmsUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<ICMSProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        ...
        headerName: 'Estado de Recebedor',
        filterParams: filterParametersText,
      },
      {
        initialWidth: 260,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    const { data: result } = await refetch({
      ...
      where: { recipient_state: { contains: search } },
    });

    setRowData(result.getAllICMS ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllICMS ?? []);

        return {
          ...fetchMoreResult,
          ICMS: [
            ...(previous.getAllICMS ?? []),
            ...(fetchMoreResult.getAllICMS ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateICMS = async () => {
    setIsFetchingUpdateICMS(true);
    ...
    try {
      ...
      const isPlural = data?.updateManyIcms && data.updateManyIcms.length > 1;

      toast.success(
        `ICMS${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a ICMS!', {
      ...
      setIsFetchingUpdateICMS(false);
    }
  };

  const handleDeleteICMS = async () => {
    setIsFetchingDeleteICMS(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteICMS({
        variables: { ids },
      });

      const isPlural = data?.deleteManyIcms && data?.deleteManyIcms.length > 1;

      toast.success(
        `ICMS${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a ICMS!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteICMS(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<ICMSProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<ICMSProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as IcmsWhereInput;
    const keys = Object.keys(filter) as ICMSKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[item as keyof Omit<ICMSProps, '__typename' | 'state_origin'>] = {
    ...
    setRowData(data.getAllICMS ?? []);
  };

  const rowClassRules: RowClassRules<ICMSProps> = {
  ...
  useEffect(() => {
    if (dataICMS?.getAllICMS ?? []) {
      setRowData(dataICMS?.getAllICMS ?? []);
    }
  }, [isLoading]);
      ...
      <div className="flex flex-col">
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      variant="label"
                      color="success"
                      onClick={handleUpdateICMS}
                      className="min-w-[10.564rem]"
                      isLoading={isFetchingUpdateICMS}
                      isDisabled={isFetchingUpdateICMS}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/icms/new">
              <Button variant="label">Adicionar ICMS</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingICMS,
                    'opacity-0 pointer-events-none': !isFetchingICMS,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/icms/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              color="danger"
                              variant="label"
                              onClick={handleDeleteICMS}
                              className="min-w-[5.806rem]"
                              isLoading={isFetchingDeleteICMS}
                              isDisabled={isFetchingDeleteICMS}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataICMS.countIcms / pageSize,
                )} página${
                  Math.ceil(dataICMS.countIcms / pageSize) > 1 ? 's' : ''
                } e ${dataICMS?.countIcms} registro${
                  dataICMS?.countIcms > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                activeIndex={page}
                isLoading={isFetchingICMS}
                onChangeIndex={handleFetchMorePage}
                length={Math.ceil(dataICMS.countIcms / pageSize)}
