...
import { createPhysicalCustomerOrderSchema } from './schema';
import {
  type CreatePhysicalCustomerOrderProps,
  type CreatePhysicalCustomerOrderInputProps,
  type CreatePhysicalCustomerOrderOutputProps,
} from './types';

const CreatePhysicalCustomerOrderRef: ForwardRefRenderFunction<
  ElementRef<'div'>,
  CreatePhysicalCustomerOrderProps
> = ({ className, ...props }, ref) => {
  ...
  const [createPhysicalCustomerOrder] =
    useCreatePhysicalCustomerOrderMutation();
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreatePhysicalCustomerOrderInputProps>({
    resolver: zodResolver(createPhysicalCustomerOrderSchema),
    defaultValues: {
      expenses: [],
      carrier: { id: '', description: '' },
      quoteTable: { id: '', description: '' },
      physicalCustomer: { id: '', description: '' },
    },
  });
  ...
  const handleCreate = async (data: CreatePhysicalCustomerOrderOutputProps) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao criar Pedido Físico!', {
        ...
        <div className="flex w-full justify-center sm:justify-end">
          <Button
            onClick={handleSubmit((values: unknown) => {
              const data = values as CreatePhysicalCustomerOrderOutputProps;
...
export const CreatePhysicalCustomerOrder = forwardRef(
  CreatePhysicalCustomerOrderRef,
);
