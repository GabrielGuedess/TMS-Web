...
import {
  ...
  type IncidentUpdateManyInput,
  useDeleteManyIncidentMutation,
  useUpdateManyIncidentMutation,
...
import {
  ...
  type DataTableIncidentsProps,
} from './types';

dayjs.locale('pt-br');
dayjs.extend(LocalizedFormat);

export const DataTableIncidents = ({ ...props }: DataTableIncidentsProps) => {
  const [isFetchingIncidents, setIsFetchingIncidents] = useState(false);
  const [isFetchingUpdateIncidents, setIsFetchingUpdateIncidents] =
    useState(false);
  const [isFetchingDeleteIncidents, setIsFetchingDeleteIncidents] =
    useState(false);

  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(25);

  const [rowData, setRowData] = useState<IncidentProps[]>([]);

  const [isOpenDelete, setIsOpenDelete] = useState(false);
  const [isOpenUpdate, setIsOpenUpdate] = useState(false);
  const [isOpenDiscardChanges, setIsOpenDiscardChanges] = useState(false);

  const [selectedRows, setSelectedRows] = useState<IncidentProps[]>([]);
  const [editedRows, setEditedRows] = useState<EditRowProps>({
    newData: [],
    oldData: [],
  } as EditRowProps);

  const [update] = useUpdateManyIncidentMutation();
  const [deleteIncidents] = useDeleteManyIncidentMutation();
  ...
  const valueSetter = (parameters: ValueSetterParams<IncidentProps>) => {
    const { id } = parameters.data;
    const column = parameters.column.getId() as IncidentKeyProps;

    if (parameters.data[column] !== parameters.newValue) {
      const newData: IncidentProps = {
        ...parameters.data,
      ...
      const newEditData: OptionalWithIdProps<IncidentUpdateManyInput> = {
        id,
  ...
  const colDefs = useMemo<ColDef<IncidentProps>[]>(
    () => [
      {
        field: 'id',
        filter: true,
      ...
      },
      {
        filter: true,
        editable: true,
        field: 'order_process_id',
        headerName: 'Processo do Pedido',
        filterParams: filterParametersText,
      },
      {
        filter: true,
  ...
  const handleSearch = async ({ search }: { search: string }) => {
    ...
    setRowData(result.getAllIncidents ?? []);
  ...
  const handleFetchMoreSize = async (currentSize: number) => {
    ...
    await fetchMore({
      ...
      updateQuery: (previous, { fetchMoreResult }) => {
        ...
        setRowData(fetchMoreResult.getAllIncidents ?? []);

        return {
          ...fetchMoreResult,
          Incidents: [
            ...(previous.getAllIncidents ?? []),
            ...(fetchMoreResult.getAllIncidents ?? []),
          ],
        };
      },
    });
  ...
  const handleUpdateIncidents = async () => {
    setIsFetchingUpdateIncidents(true);
    ...
    try {
      ...
      const isPlural =
        data?.updateManyIncident && data.updateManyIncident.length > 1;

      toast.success(
        `Incidente${isPlural ? 's' : ''} atualizado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao atualizar a Incidente!', {
      ...
      setIsFetchingUpdateIncidents(false);
    }
  };

  const handleDeleteIncidents = async () => {
    setIsFetchingDeleteIncidents(true);

    try {
      const ids = selectedRows.map(item => item.id);

      const { data } = await deleteIncidents({
        variables: { ids },
      });

      const isPlural =
        data?.deleteManyIncident && data?.deleteManyIncident.length > 1;

      toast.success(
        `Incidente${isPlural ? 's' : ''} deletado${
          isPlural ? 's' : ''
        } com sucesso!`,
      );
      ...
      if (error instanceof ApolloError) {
        toast.error('Erro ao deletar a Incidente!', {
          description: error.message,
        });
      ...
      setIsFetchingDeleteIncidents(false);
    }
  };

  const onSelectionChanged = (event: SelectionChangedEvent<IncidentProps>) => {
    setSelectedRows(event.api.getSelectedRows());
  };

  const onFilterChanged = async (event: FilterChangedEvent<IncidentProps>) => {
    const filter = event.api.getFilterModel() as FilterProps;

    const where = {} as IncidentWhereInput;
    const keys = Object.keys(filter) as IncidentKeyProps[];
    ...
    keys.forEach(item => {
      ...
      if (
        ...
        where[
          item as keyof Omit<IncidentProps, '__typename' | 'legalPersonId'>
        ] = {
    ...
    setRowData(data.getAllIncidents ?? []);
  };

  const rowClassRules: RowClassRules<IncidentProps> = {
  ...
  useEffect(() => {
    if (dataIncidents?.getAllIncidents ?? []) {
      setRowData(dataIncidents?.getAllIncidents ?? []);
    }
  }, [isLoading]);
        ...
        <div className="relative flex flex-wrap-reverse items-center justify-between gap-2 bg-white-lilac-50 p-4 transition-all dark:bg-smoke-950 md:flex-row md:flex-wrap md:space-y-0">
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                  ...
                  <div className="mt-6 flex justify-end">
                    <Button
                      ...
                      onClick={handleUpdateIncidents}
                      isLoading={isFetchingUpdateIncidents}
                      isDisabled={isFetchingUpdateIncidents}
                    >
                      Salvar alterações
                    </Button>
                  </div>
          ...
          <div className="relative flex flex-1 gap-3 md:flex-initial">
            <Link href="/dashboard/incidents/new">
              <Button variant="label">Adicionar Incidente</Button>
            </Link>
            ...
            <div className="ag-theme-quartz relative h-[31.25rem]">
              <div
                className={clsx(
                  ...
                  {
                    'opacity-1 pointer-events-auto': isFetchingIncidents,
                    'opacity-0 pointer-events-none': !isFetchingIncidents,
                  },
                )}
              >
                <PreLoader />
                ...
                <Popover.Portal>
                  ...
                  >
                    {selectedRows.length === 1 && (
                      <Link
                        href={`/dashboard/incidents/${selectedRows[0].id}/general`}
                        ...
                        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-40 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 animate-contentShow rounded-md bg-white-lilac-50 p-9 transition-all dark:bg-smoke-950">
                          ...
                          <div className="mt-6 flex justify-end gap-3">
                            ...
                            <Button
                              ...
                              isLoading={isFetchingDeleteIncidents}
                              isDisabled={isFetchingDeleteIncidents}
                            >
                              Deletar
                            </Button>
                          </div>
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-4 p-4 md:mt-0 md:flex-row">
              <span className="text-[0.813rem] text-comet-500 transition-all dark:text-dark-300">
                {`Mostrando ${page + 1} de ${Math.ceil(
                  dataIncidents.countIncident / pageSize,
                )} página${
                  Math.ceil(dataIncidents.countIncident / pageSize) > 1
                    ? 's'
                    : ''
                } e ${dataIncidents?.countIncident} registro${
                  dataIncidents?.countIncident > 1 ? 's' : ''
                }`}
              </span>
              ...
              <PageIndicator
                ...
                length={Math.ceil(dataIncidents.countIncident / pageSize)}
              />
            </div>
          </>
        )}
